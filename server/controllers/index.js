...
const searchNameDog = (req, res) => {
  ...
  return Dog.findByName(req.query.name, (err, doc) => {
    ...
    const dog = { name: lastAddedDog.name, breed: lastAddedDog.breed, age: lastAddedDog.age };
    savePromise.then(() => res.json(dog));
    savePromise.catch(err_ => res.json({ err_ }));

    // if a match, send the match back
    // return res.json({ name: doc.name, breed: doc.breed, age: doc.age });
    return { name: doc.name, breed: doc.breed, age: doc.age };
  });
};
